...
const clusterOidcProviderUrl = cluster.core.oidcProvider!.url
const clusterOidcProviderArn = cluster.core.oidcProvider!.arn
const ebsCsiDriverName = 'my-eks-ebs-csi-driver'
const role = pulumi.all([clusterOidcProviderUrl, clusterOidcProviderArn]).apply(([url, arn]) =>
            ...
            Statement: [{
                ...
                Condition: {
                    StringEquals: {
                        ...
                        [`${url.replace('https://', '')}:sub`]: `system:serviceaccount:kube-system:ebs-csi-controller-sa`,
                    }
                }
            }]
        },
...
const attachment = role.apply(r => new aws.iam.RolePolicyAttachment(ebsCsiDriverName, {
    role: r,
    policyArn: aws.iam.ManagedPolicy.AmazonEBSCSIDriverPolicy,
}))
const addon = role.apply(r =>
